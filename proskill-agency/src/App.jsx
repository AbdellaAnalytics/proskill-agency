...
const sb = {
  ...
  async saveData(workspaceOwnerId, data, adminEmail) {
    ...
    for (let attempt = 1; attempt <= 3; attempt++) {
      try {
        // 1. Try PATCH. Use Prefer: count=exact so we know if it actually updated anything
        const r = await this.req(
          "/rest/v1/proskill_workspace?workspace_key=eq." + encodeURIComponent(key),
          { method: "PATCH", prefer: "return=representation", body }
        );
        // If PATCH succeeded and updated at least one row, we're good
        if (r.ok && Array.isArray(r.data) && r.data.length > 0) {
          // Also upsert the email index so members can find this workspace
          if (adminEmail) {
            await this.upsertEmailIndex(workspaceOwnerId, data, adminEmail).catch(() => {});
          }
          return { ok: true };
        }
        // PATCH returned 0 rows — need to INSERT
        const r2 = await this.req(
          "/rest/v1/proskill_workspace",
          { method: "POST", prefer: "return=minimal", body: JSON.stringify({ owner_id: workspaceOwnerId, workspace_key: key, data }) }
        );
        if (r2.ok) {
          if (adminEmail) {
            await this.upsertEmailIndex(workspaceOwnerId, data, adminEmail).catch(() => {});
        ...
        // Insert might fail due to duplicate key (race condition) — retry PATCH
      } catch (e) { /* retry */ }
      if (attempt < 3) {
  ...
  async upsertEmailIndex(workspaceOwnerId, data, adminEmail) {
    ...
    return await this.req(
      "/rest/v1/proskill_workspace",
      { method: "POST", prefer: "return=minimal", body: JSON.stringify({ owner_id: workspaceOwnerId, workspace_key: emailKey, data }) }
    );
  },
};
