...
export default function App() {
  ...
  const importAdobeFromExcel = (parsedRows, mode = "smart") => {
    if (!isAdmin) { alert("Admin only"); return; }
    let addedAccs = 0, addedRentals = 0, skipped = 0, updatedAccs = 0;
    const newAccs = [...adobeAccounts];
    const newRentals = [...adobeRentals];
    parsedRows.forEach(row => {
      ...
      if (!acc) {
        ...
        // Overwrite mode: update all fields including credentials + account dates
      ...
      if (row.soldTo && row.customerPlan) {
        ...
        // Check for duplicate (same account + same customer + same end)
        const dup = newRentals.find(r => r.accountId === acc.id && r.endDate === custEndDate && r.customer === row.soldTo);
        if (!dup && custEndDate) {
          newRentals.push({
            ...
            endDate: custEndDate,
            planMonths,
            // Store the imported days-left for display (as opposed to recomputing daily)
            importedDaysLeft: validDays,
            importedAt: validDays !== null ? todayStr() : null,
            orderNumber: row.orderNumber || "",
            status: (custEndDate && isAfterDate(custEndDate, todayStr())) || validDays > 0 ? "active" : "completed",
    ...
    addLog("📤 Imported Adobe: " + addedAccs + " new accounts, " + updatedAccs + " updated, " + addedRentals + " rentals, " + skipped + " skipped");
    return { addedAccs, updatedAccs, addedRentals, skipped };
  };
            ...
            <div>
              ...
              <div style={{ display: "flex", gap: 4, marginBottom: 14, borderBottom: "1px solid " + t.border, overflowX: "auto" }}>
                {[
                  ...
                  { id: "active",   l: "🟢 Active Rentals (" + activeRentals.length + ")" },
                  { id: "history",  l: "📜 History (" + historyRentals.length + ")" },
                ].map(s => (
                  <button
                    key={s.id}
        ...
        {showAdobeImport && (
          <AdobeImportModal
            ...
            onImport={(rows, mode) => {
              ...
              alert("✅ Import complete!\n\n" + res.addedAccs + " accounts added\n" + res.addedRentals + " rentals created\n" + res.skipped + " rows skipped");
            }}
          />
        )}
