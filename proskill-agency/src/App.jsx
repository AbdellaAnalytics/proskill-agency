...
export default function App() {
  ...
  // ─── Adobe Tracker (Google Sheet sync via Vercel API) ───
  const [adobeSheetAccounts, setAdobeSheetAccounts] = useState([]);
  const [adobeSheetStatus, setAdobeSheetStatus] = useState("idle"); // idle | loading | ok | error
  const [adobeSheetError, setAdobeSheetError] = useState("");
  const [adobeSheetFetchedAt, setAdobeSheetFetchedAt] = useState(null);
  const [adobeTrackerSearch, setAdobeTrackerSearch] = useState("");
  const [adobeTrackerFilter, setAdobeTrackerFilter] = useState("all"); // all | active | expired | expiring | sold | available
  ...
  // ─── Fetch Adobe Tracker sheet from Vercel serverless function ───
  const fetchAdobeSheet = useCallback(async () => {
    setAdobeSheetStatus("loading");
    setAdobeSheetError("");
    try {
      const r = await fetch("/api/adobe-sheet", { cache: "no-store" });
      if (!r.ok) {
      ...
      setAdobeSheetAccounts(json.accounts || []);
      setAdobeSheetFetchedAt(json.fetchedAt);
      setAdobeSheetStatus("ok");
    } catch (e) {
      console.error("[ProSkill] Adobe sheet fetch failed:", e);
      setAdobeSheetError(e.message || "Unknown error");
      setAdobeSheetStatus("error");
    }
  }, []);

  // Auto-fetch sheet once after auth, then every 5 minutes
  useEffect(() => {
    if (authStatus !== "app") return;
    fetchAdobeSheet();
    const id = setInterval(fetchAdobeSheet, 5 * 60 * 1000);
    return () => clearInterval(id);
  }, [authStatus, fetchAdobeSheet]);
  ...
  const addSaleEntry = () => {
    ...
    lines.forEach((L, idx) => {
      ...
      const entry = {
        ...
        // Adobe Tracker linkage
        adobeAccountEmail: L.service === "Adobe" && L.adobeAccountEmail ? L.adobeAccountEmail.toLowerCase().trim() : null,
      };
      if (available) {
  ...
  const alerts = useMemo(() => {
    ...
    // Adobe Tracker alerts — for sales linked to sheet accounts
    if (isAdmin && Array.isArray(adobeSheetAccounts) && adobeSheetAccounts.length > 0) {
      baseSales.forEach(s => {
        if (s.service !== "Adobe" || !s.adobeAccountEmail || !s.done || s.refunded) return;
        const acc = adobeSheetAccounts.find(a => a.email === (s.adobeAccountEmail || "").toLowerCase());
        if (!acc || !acc.endDate) return;
        const d = Math.ceil((new Date(acc.endDate) - new Date()) / 864e5);
        if (d < 0) {
          allN.push({ id: "atexp" + s.id, t: "danger", m: "🎯 Adobe acc EXPIRED: " + s.customer + " (" + Math.abs(d) + "d ago)", saleId: s.id });
        } else if (d <= 7) {
          allN.push({ id: "atexp" + s.id, t: "warn", m: "🎯 Adobe expiring: " + s.customer + " (" + d + "d)", saleId: s.id });
        }
        // Also flag if status from sheet is not Active
        if (acc.status && acc.status.toLowerCase() !== "active") {
          allN.push({ id: "atst" + s.id, t: "warn", m: "🎯 Adobe status: " + s.customer + " → " + acc.status, saleId: s.id });
        }
      });
    }
  ...
  }, [sales, scopedSales, svcNames, stockStatsByProduct, adobePendingReminders, adobeOverdue, isAdmin, dismissedN, adobeSheetAccounts]);
  ...
  return (
    <div style={{
      ...
      <div style={{
          ...
          >
            ...
            >
              ...
              <div style={{ background: t.cardBg2, borderRadius: 10, padding: 14, marginBottom: 14, fontSize: t.fs.base, lineHeight: 1.6 }}>
                ...
                {/* Adobe Tracker linked account info */}
                {selSale.service === "Adobe" && selSale.adobeAccountEmail && (() => {
                  const acc = adobeSheetAccounts.find(a => a.email === selSale.adobeAccountEmail.toLowerCase());
                  if (!acc) {
                    return (
                      <div style={{ marginTop: 8, padding: 8, background: t.dark ? "#422006" : "#fffbeb", borderRadius: 6, fontSize: t.fs.sm, color: t.warning }}>
                        🎯 Adobe acc: <strong>{selSale.adobeAccountEmail}</strong> (not found in tracker — refresh tracker?)
                      </div>
                    );
                  }
                  const d = acc.endDate ? Math.ceil((new Date(acc.endDate) - new Date()) / 864e5) : null;
                  ...
                  return (
                    <div style={{ marginTop: 8, padding: 10, background: t.cardBg2, borderRadius: 8, fontSize: t.fs.sm }}>
                      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 4 }}>
                        <strong style={{ color: t.primary }}>🎯 Adobe Tracker</strong>
                        <span style={{ color: stColor, fontWeight: 700, fontSize: t.fs.xs }}>
                          {d === null ? "—" : d < 0 ? "Expired " + Math.abs(d) + "d ago" : d + " days left"}
                        </span>
                      </div>
                      ...
                      <div style={{ fontSize: t.fs.xs, color: t.textMuted }}>
                        Plan: {acc.customerPlan || "—"} · Ends: {acc.endDate || "—"}
                      </div>
                    </div>
                  );
                })()}
              ...
              <div style={{ ...t.card, marginBottom: 14, border: "2px solid " + t.primary }}>
                ...
                {newSale.lines.map((L, idx) => (
                  <div key={idx} style={{
                    ...
                    {/* Adobe-only: account email picker linked to tracker */}
                    {L.service === "Adobe" && (() => {
                      const linkedAcc = adobeSheetAccounts.find(a => a.email === (L.adobeAccountEmail || "").toLowerCase());
                      const datalistId = "adobeAccts-" + idx;
                      return (
                        <div style={{ marginTop: 8 }}>
                          <label style={t.label}>🎯 Adobe Account Email (link to tracker)</label>
                          <input
                            list={datalistId}
                            value={L.adobeAccountEmail || ""}
                            onChange={e => updateLine(idx, { adobeAccountEmail: e.target.value.toLowerCase().trim() })}
                            placeholder="Type or pick from tracker..."
                            style={t.input}
                          />
                          <datalist id={datalistId}>
                            {adobeSheetAccounts.filter(a => !a.soldTo).map(a => (
                              <option key={a.rowIndex} value={a.email}>
                                {a.email} — {a.customerPlan || "—"} — ends {a.endDate || "?"}
                              </option>
                            ))}
                          </datalist>
                          {linkedAcc && (
                            <div style={{
                              ...
                              background: t.dark ? "rgba(42,157,143,0.15)" : "#e8f4f2",
                              fontSize: t.fs.xs,
                            }}>
                              <strong style={{ color: t.primary }}>✓ Linked:</strong> {linkedAcc.customerPlan || "—"} · ends {linkedAcc.endDate || "?"}
                              {linkedAcc.endDate && (() => {
                                const d = Math.ceil((new Date(linkedAcc.endDate) - new Date()) / 864e5);
                                return <span style={{ color: d < 0 ? t.danger : d <= 7 ? "#f97316" : t.success, fontWeight: 700 }}> ({d < 0 ? Math.abs(d) + "d ago" : d + "d left"})</span>;
                              })()}
                            </div>
                          )}
                          {!linkedAcc && L.adobeAccountEmail && adobeSheetStatus === "ok" && (
                            <div style={{ fontSize: t.fs.xs, color: t.warning, marginTop: 4 }}>
                              ⚠️ Email not in tracker (will save anyway)
                            </div>
                          )}
                        </div>
                      );
        ...
        {/* ═══════════════════════════════════════════════════════════════
             🎯 ADOBE TRACKER TAB — live sync from Google Sheet
             ═══════════════════════════════════════════════════════════════ */}
        {tab === "adobeTracker" && (() => {
          const now = new Date();
          // Apply filters
          const filtered = adobeSheetAccounts.filter(a => {
            if (adobeTrackerSearch) {
              const q = adobeTrackerSearch.toLowerCase();
              if (!a.email.includes(q)
                && !(a.orderNumber || "").toLowerCase().includes(q)
                && !(a.soldTo || "").toLowerCase().includes(q)
                && !(a.customerPlan || "").toLowerCase().includes(q)) return false;
            }
            const daysToEnd = a.endDate ? Math.ceil((new Date(a.endDate) - now) / 864e5) : null;
            if (adobeTrackerFilter === "active") {
              if (!(a.status && a.status.toLowerCase() === "active")) return false;
            } else if (adobeTrackerFilter === "expired") {
              if (!(daysToEnd !== null && daysToEnd < 0)) return false;
            } else if (adobeTrackerFilter === "expiring") {
              if (!(daysToEnd !== null && daysToEnd >= 0 && daysToEnd <= 7)) return false;
            } else if (adobeTrackerFilter === "sold") {
              if (!a.soldTo) return false;
            } else if (adobeTrackerFilter === "available") {
              if (a.soldTo) return false;
            }
            return true;
          });
          ...
          // Summary stats
          const total = adobeSheetAccounts.length;
          const activeCount = adobeSheetAccounts.filter(a => (a.status || "").toLowerCase() === "active").length;
          const expiredCount = adobeSheetAccounts.filter(a => {
            const d = a.endDate ? Math.ceil((new Date(a.endDate) - now) / 864e5) : null;
            return d !== null && d < 0;
          }).length;
          const expiringCount = adobeSheetAccounts.filter(a => {
            const d = a.endDate ? Math.ceil((new Date(a.endDate) - now) / 864e5) : null;
            return d !== null && d >= 0 && d <= 7;
          }).length;
          const soldCount = adobeSheetAccounts.filter(a => a.soldTo).length;
            ...
            <div>
              <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 14, flexWrap: "wrap", gap: 10 }}>
                ...
                <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
                  {adobeSheetFetchedAt && (
                    <span style={{ fontSize: t.fs.xs, color: t.textMuted }}>
                      Synced {new Date(adobeSheetFetchedAt).toLocaleTimeString()}
                    </span>
                  )}
                  <button
                    onClick={fetchAdobeSheet}
                    disabled={adobeSheetStatus === "loading"}
                    style={{ ...t.btnPrimary, opacity: adobeSheetStatus === "loading" ? 0.5 : 1 }}
                  >
                    {adobeSheetStatus === "loading" ? "⏳ Loading..." : "🔄 Refresh"}
                  </button>
                </div>
              </div>

              {/* Status banner */}
              {adobeSheetStatus === "error" && (
                <div style={{
                  ...t.card, marginBottom: 14,
                  background: t.dark ? "#450a0a" : "#fef2f2",
                  borderLeft: "3px solid " + t.danger,
                }}>
                  <strong style={{ color: t.danger }}>⚠️ Sync error:</strong>
                  <div style={{ fontSize: t.fs.sm, marginTop: 6, color: t.textMuted, wordBreak: "break-word" }}>
                    {adobeSheetError}
                  </div>
                  <div style={{ fontSize: t.fs.xs, marginTop: 8, color: t.textMuted }}>
                    Check Vercel environment variables: GOOGLE_SA_CLIENT_EMAIL, GOOGLE_SA_PRIVATE_KEY, GOOGLE_SHEET_ID
                  </div>
                </div>
              )}

              {adobeSheetStatus === "idle" || (adobeSheetStatus === "loading" && total === 0) ? (
                <div style={{ ...t.card, textAlign: "center", padding: 40, color: t.textMuted }}>
                  <div style={{ fontSize: 40, marginBottom: 10 }}>⏳</div>
                  <p>{adobeSheetStatus === "loading" ? "Loading sheet..." : "Click Refresh to load Adobe tracker"}</p>
                </div>
              ) : total === 0 && adobeSheetStatus === "ok" ? (
                <div style={{ ...t.card, textAlign: "center", padding: 40, color: t.textMuted }}>
                  <p>Sheet is empty or no rows match.</p>
                </div>
              ) : (
                <>
                  {/* Summary cards */}
                  <div style={{ display: "grid", gridTemplateColumns: isMobile ? "1fr 1fr" : "repeat(5, 1fr)", gap: t.sp.md, marginBottom: 14 }}>
                    <div onClick={() => setAdobeTrackerFilter("all")} style={{ ...t.card, textAlign: "center", padding: 14, cursor: "pointer", borderTop: "3px solid " + t.primary, background: adobeTrackerFilter === "all" ? (t.dark ? "rgba(42,157,143,0.15)" : "#e8f4f2") : t.cardBg }}>
                      <div style={{ fontSize: t.fs.xs, color: t.textMuted, fontWeight: 700 }}>TOTAL</div>
                      <div style={{ fontSize: t.fs.xl, fontWeight: 900, color: t.primary }}>{total}</div>
                    </div>
                    <div onClick={() => setAdobeTrackerFilter("active")} style={{ ...t.card, textAlign: "center", padding: 14, cursor: "pointer", borderTop: "3px solid " + t.success, background: adobeTrackerFilter === "active" ? (t.dark ? "rgba(34,197,94,0.15)" : "#f0fdf4") : t.cardBg }}>
                      <div style={{ fontSize: t.fs.xs, color: t.textMuted, fontWeight: 700 }}>🟢 ACTIVE</div>
                      <div style={{ fontSize: t.fs.xl, fontWeight: 900, color: t.success }}>{activeCount}</div>
                    </div>
                    <div onClick={() => setAdobeTrackerFilter("expiring")} style={{ ...t.card, textAlign: "center", padding: 14, cursor: "pointer", borderTop: "3px solid #f97316", background: adobeTrackerFilter === "expiring" ? (t.dark ? "rgba(249,115,22,0.15)" : "#fff7ed") : t.cardBg }}>
                    ...
                    <div onClick={() => setAdobeTrackerFilter("expired")} style={{ ...t.card, textAlign: "center", padding: 14, cursor: "pointer", borderTop: "3px solid " + t.danger, background: adobeTrackerFilter === "expired" ? (t.dark ? "rgba(239,68,68,0.15)" : "#fef2f2") : t.cardBg }}>
                      <div style={{ fontSize: t.fs.xs, color: t.textMuted, fontWeight: 700 }}>🔴 EXPIRED</div>
                      <div style={{ fontSize: t.fs.xl, fontWeight: 900, color: t.danger }}>{expiredCount}</div>
                    </div>
                    <div onClick={() => setAdobeTrackerFilter("available")} style={{ ...t.card, textAlign: "center", padding: 14, cursor: "pointer", borderTop: "3px solid #8b5cf6", background: adobeTrackerFilter === "available" ? (t.dark ? "rgba(139,92,246,0.15)" : "#faf5ff") : t.cardBg }}>
                      <div style={{ fontSize: t.fs.xs, color: t.textMuted, fontWeight: 700 }}>📦 AVAILABLE</div>
                      <div style={{ fontSize: t.fs.xl, fontWeight: 900, color: "#8b5cf6" }}>{availCount}</div>
                    </div>
                  </div>
                  ...
                  <div style={{ ...t.card, marginBottom: 12, padding: 12 }}>
                    <div style={{ display: "grid", gridTemplateColumns: isMobile ? "1fr" : "2fr 1fr auto", gap: 8 }}>
                      <input
                        placeholder="🔍 Search email, order #, customer..."
                        value={adobeTrackerSearch}
                        onChange={e => setAdobeTrackerSearch(e.target.value)}
                        style={t.input}
                      />
                      <select value={adobeTrackerFilter} onChange={e => setAdobeTrackerFilter(e.target.value)} style={t.input}>
                      ...
                      {(adobeTrackerSearch || adobeTrackerFilter !== "all") && (
                        <button onClick={() => { setAdobeTrackerSearch(""); setAdobeTrackerFilter("all"); }} style={{ ...t.btnGhost, padding: "8px 14px" }}>✕ Clear</button>
                      )}
                    </div>
                    <div style={{ fontSize: t.fs.sm, color: t.textMuted, marginTop: 8 }}>
                      Showing <strong style={{ color: t.primary }}>{filtered.length}</strong> of {total} accounts
                    </div>
                  </div>
                  ...
                  {filtered.length === 0 ? (
                    ...
                    filtered.map(a => {
                      const daysToEnd = a.endDate ? Math.ceil((new Date(a.endDate) - now) / 864e5) : null;
                      const stColor = daysToEnd === null ? t.textMuted
                        : daysToEnd < 0 ? t.danger
                        : daysToEnd <= 7 ? "#f97316"
                        : t.success;
                      const linkedSales = sales.filter(s => (s.adobeAccountEmail || "").toLowerCase() === a.email);
                      return (
                        <div key={a.rowIndex} style={{
                          ...t.card, marginBottom: 10, padding: 14,
                          borderLeft: "3px solid " + stColor,
                        }}>
                          <div style={{ display: "flex", justifyContent: "space-between", marginBottom: 8, gap: 8 }}>
                            <div style={{ minWidth: 0, flex: 1 }}>
                              <strong style={{ fontSize: t.fs.sm, wordBreak: "break-word" }}>{a.email}</strong>
                              <div style={{ fontSize: t.fs.xs, color: t.textMuted }}>{a.customerPlan || "—"}</div>
                            </div>
                            <div style={{ textAlign: "right", flexShrink: 0 }}>
                              <div style={{ fontSize: t.fs.sm, fontWeight: 700, color: stColor }}>
                                {daysToEnd === null ? "—" : daysToEnd < 0 ? Math.abs(daysToEnd) + "d ago" : daysToEnd + "d left"}
                              </div>
                              <div style={{ fontSize: t.fs.xs, color: t.textMuted }}>{a.endDate || "no date"}</div>
                            </div>
                          </div>
                          {a.soldTo && (
                            <div style={{ fontSize: t.fs.xs, color: t.textMuted }}>📞 Sold to: {a.soldTo}</div>
                          )}
                          {a.orderNumber && (
                            <div style={{ fontSize: t.fs.xs, color: t.textMuted }}>📋 Order: {a.orderNumber}</div>
                          )}
                          {linkedSales.length > 0 && (
                            <div style={{ fontSize: t.fs.xs, color: t.primary, marginTop: 4 }}>
                              🔗 {linkedSales.length} linked sale{linkedSales.length > 1 ? "s" : ""}
                            </div>
                          )}
                        </div>
                      );
                        ...
                        <table style={{ width: "100%", borderCollapse: "collapse", minWidth: 900, fontSize: t.fs.sm }}>
                          <thead>
                            <tr style={{ background: t.cardBg2 }}>
                              {["Email", "Plan", "Start", "End", "Status", "Days", "Sold To", "Order", "Linked"].map(h => (
                                <th key={h} style={{ padding: "10px 12px", textAlign: "left", fontSize: t.fs.xs, color: t.textMuted, fontWeight: 700, textTransform: "uppercase" }}>{h}</th>
                              ))}
                            </tr>
                          </thead>
                          <tbody>
                            {filtered.map(a => {
                              ...
                              const stColor = daysToEnd === null ? t.textMuted
                                ...
                                : daysToEnd <= 7 ? "#f97316"
                                : t.success;
                              const rowBg = daysToEnd !== null && daysToEnd < 0 ? (t.dark ? "#1c0e0e" : "#fef2f2")
                                : daysToEnd !== null && daysToEnd <= 7 ? (t.dark ? "#1f1610" : "#fff7ed")
                                : "transparent";
                              const linkedSales = sales.filter(s => (s.adobeAccountEmail || "").toLowerCase() === a.email);
                              return (
                                <tr key={a.rowIndex} style={{ borderBottom: "1px solid " + t.border, background: rowBg }}>
                                  <td style={{ padding: "10px 12px", wordBreak: "break-word" }}>{a.email}</td>
                                  <td style={{ padding: "10px 12px" }}>{a.customerPlan || "—"}</td>
                                  <td style={{ padding: "10px 12px", color: t.textMuted }}>{a.startDate || "—"}</td>
                                  <td style={{ padding: "10px 12px", fontWeight: 600 }}>{a.endDate || "—"}</td>
                                  <td style={{ padding: "10px 12px" }}>
                                    <span style={{
                                      padding: "2px 10px", borderRadius: 20, fontSize: t.fs.xs, fontWeight: 700,
                                      background: (a.status || "").toLowerCase() === "active" ? t.success + "22" : t.textMuted + "22",
                                      color: (a.status || "").toLowerCase() === "active" ? t.success : t.textMuted,
                                    }}>{a.status || "?"}</span>
                                  </td>
                                  <td style={{ padding: "10px 12px", fontWeight: 700, color: stColor }}>
                                    {daysToEnd === null ? "—" : daysToEnd < 0 ? Math.abs(daysToEnd) + "d ago" : daysToEnd + "d"}
                                  </td>
                                  <td style={{ padding: "10px 12px", color: t.textMuted, fontSize: t.fs.xs }}>{a.soldTo || "—"}</td>
                                  <td style={{ padding: "10px 12px", color: t.textMuted, fontSize: t.fs.xs }}>{a.orderNumber || "—"}</td>
                                  <td style={{ padding: "10px 12px" }}>
                                    {linkedSales.length > 0 ? (
                                      <span style={{ color: t.primary, fontWeight: 700, fontSize: t.fs.xs }}>
                                        🔗 {linkedSales.length}
                                      </span>
                                    ) : (
                                      <span style={{ color: t.textMuted, fontSize: t.fs.xs }}>—</span>
                                    )}
                                  </td>
                                </tr>
                              );
        ...
        {tab === "reports" && (
          <div>
            ...
            {["sales", "profit", "payment", "team", "customers", "inventory", "insights", "bi", "refunds", "statuses", "suppliers"].includes(repTab) && (
              <div style={{ ...t.card, marginBottom: 14, padding: 12 }}>
