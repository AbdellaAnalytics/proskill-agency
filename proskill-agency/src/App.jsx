      ...
      <div style={{
        ...
        {tab === "reports" && (
          <div>
            ...
            {["sales", "profit", "payment", "team", "customers", "inventory", "insights"].includes(repTab) && (
            ...
            {repTab === "customers" && (() => {
              ...
              repDone.forEach(a => {
                ...
                if (!k) return;
                if (!byCust[k]) byCust[k] = { name: k, phone: a.customerPhone || "", email: a.customerEmail || "", count: 0, rev: 0, products: new Set() };
                byCust[k].count++;
                byCust[k].rev += (a.priceEGP || a.price || 0);
                byCust[k].products.add(a.service);
              });
              const custs = Object.values(byCust).map(c => ({ ...c, products: Array.from(c.products) })).sort((a, b) => b.rev - a.rev);
              ...
              return (
                <div>
                  ...
                  <div style={t.card}>
                    <h3 style={{ margin: "0 0 12px", fontSize: t.fs.md, fontWeight: 700 }}>🏆 Top Customers (by lifetime value)</h3>
                    {custs.length === 0 ? (
                      <p style={{ color: t.textMuted, margin: 0 }}>No customers in this period.</p>
                    ) : custs.slice(0, 20).map((c, i) => (
                      <div key={c.name} style={{
                        display: "flex", justifyContent: "space-between", alignItems: "center", gap: 8,
                        padding: "10px 0", borderBottom: "1px solid " + t.border, flexWrap: "wrap",
                      }}>
                        <div style={{ minWidth: 0, flex: 1 }}>
                          <div style={{ fontWeight: 700 }}>{i === 0 ? "🥇 " : i === 1 ? "🥈 " : i === 2 ? "🥉 " : ""}{c.name}</div>
                          <div style={{ fontSize: t.fs.xs, color: t.textMuted }}>{c.count} orders · {c.products.join(", ")}</div>
                        </div>
                        <div style={{ textAlign: "right" }}>
