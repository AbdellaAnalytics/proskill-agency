  ...
  const importAdobeFromExcel = (parsedRows, mode = "smart") => {
    ...
    parsedRows.forEach(row => {
      ...
      if (row.soldTo && row.customerPlan) {
        ...
        // Use leftForCustomer (col M) to back-calculate the rental's end date
        // If we know "X days left", customer's rental ends today + X days
        let custEndDate = null;
        let custStartDate = null;
        const daysForCust = Number(row.leftForCustomer);
        if (isFinite(daysForCust) && daysForCust > 0) {
          // Calculate ending date as today + days
          const endD = new Date();
          endD.setDate(endD.getDate() + daysForCust);
          custEndDate = endD.toISOString().slice(0, 10);
          // Calculate start as end - planMonths
        ...
        // Check for duplicate (same account + same end date)
        ...
        if (!dup && custEndDate) {
          newRentals.push({
            ...
            status: isAfterDate(custEndDate, todayStr()) ? "active" : "completed",
      ...
      <div style={{
            ...
            >
              ...
              <div style={{ background: t.cardBg2, borderRadius: 10, padding: 14, marginBottom: 14, fontSize: t.fs.base, lineHeight: 1.6 }}>
                ...
                {selSale.service === "Adobe" && selSale.adobeAccountId && (() => {
                  ...
                  if (!acc) return null;
                  const d = rental ? daysLeft(rental.endDate) : null;
        ...
        {tab === "adobeTracker" && (() => {
          ...
          const activeRentals = adobeRentals
            ...
            .map(r => ({ ...r, daysToEnd: daysLeft(r.endDate), acc: adobeAccounts.find(a => a.id === r.accountId) }))
                        ...
                        <div key={acc.id} style={{ ...t.card, marginBottom: 10, padding: 14, borderLeft: "3px solid " + stColor }}>
                          ...
                          {active && r && (
                            <div style={{ fontSize: t.fs.xs, color: t.textMuted, marginBottom: 4 }}>
                              👤 {r.customer} · {r.planMonths}m · {daysLeft(r.endDate)}d left
                            </div>
                          )}
                        ...
                        <table style={{ width: "100%", borderCollapse: "collapse", minWidth: 900, fontSize: t.fs.sm }}>
                          ...
                          <tbody>
                            {filteredAccounts.map(acc => {
                              ...
                              const accDaysLeft = acc.accountEndDate ? daysLeft(acc.accountEndDate) : null;
                              const accColor = accDaysLeft === null ? t.textMuted : accDaysLeft < 0 ? t.danger : accDaysLeft <= 30 ? "#f97316" : t.success;
                              const custDaysLeft = r ? daysLeft(r.endDate) : null;
