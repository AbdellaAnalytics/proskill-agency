...
// ═══════════════════════════════════════════════════════════════════
// 🌐 CUSTOMER SELF-CHECK PAGE — public, no login needed
// Customer picks service + enters email → sees their subscription status
// ═══════════════════════════════════════════════════════════════════
function CustomerCheckPage() {
  ...
  // Load admin's workspace (publicly readable since RLS allows authenticated reads)
  // For a true public read, we would need a public anon key path — use existing email index
  useEffect(() => {
    const load = async () => {
      try {
        // Try to read public workspace via the email index
        // We need to use a no-auth fetch since customer is not signed in
        const url = SUPABASE_URL + "/rest/v1/proskill_workspace?workspace_key=eq." + encodeURIComponent("ws_email_" + ADMIN_EMAIL.toLowerCase()) + "&select=data";
        ...
        if (r.ok) {
          ...
          if (Array.isArray(json) && json.length > 0 && json[0].data) {
            ...
            setLoadError("No data available. Please contact ProSkill on WhatsApp: " + ADMIN_WA);
          }
        } else {
          setLoadError("Service temporarily unavailable. Please try again later or contact ProSkill on WhatsApp.");
        }
      } catch (e) {
        setLoadError("Could not connect. Please check your internet connection.");
      } finally {
        setLoading(false);
      }
    };
  ...
  const services = data && Array.isArray(data.services) ? data.services.filter(s => s && s.name).map(s => s.name) : [];
  ...
  const matchedSubs = useMemo(() => {
    ...
    if (!emailLower && !phoneClean) return [];
    return sales.filter(a => {
      if (!a.done || a.refunded) return false;
      if (selService !== "all" && a.service !== selService) return false;
      const aEmail = (a.customerEmail || "").toLowerCase();
      const aPhone = (a.customerPhone || "").replace(/\D/g, "");
      const matchEmail = emailLower && aEmail === emailLower;
      const matchPhone = phoneClean && aPhone && (aPhone === phoneClean || aPhone.endsWith(phoneClean) || phoneClean.endsWith(aPhone));
      return matchEmail || matchPhone;
    });
  }, [data, searched, email, phone, selService]);
  ...
  if (loading) {
    return (
      <div style={{ background: colors.bg, color: colors.text, minHeight: "100vh", display: "flex", alignItems: "center", justifyContent: "center", fontFamily: "-apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, sans-serif" }}>
  ...
  if (loadError) {
    return (
      <div style={{ background: colors.bg, color: colors.text, minHeight: "100vh", display: "flex", alignItems: "center", justifyContent: "center", fontFamily: "-apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, sans-serif", padding: 20 }}>
        <div style={{ textAlign: "center", maxWidth: 400 }}>
          <div style={{ fontSize: 50, marginBottom: 14 }}>⚠️</div>
          <p>{loadError}</p>
          <a href={"https://wa.me/" + ADMIN_WA} style={{ ...btnStyle, display: "inline-block", textDecoration: "none", marginTop: 14 }}>📱 Contact on WhatsApp</a>
        </div>
      </div>
    );
  }
  ...
  return (
    <div style={{
      ...
      <div style={{ maxWidth: 600, margin: "0 auto" }}>
        {/* Header */}
        <div style={{ textAlign: "center", marginBottom: 30, paddingTop: isMobile ? 20 : 40 }}>
          <div style={{
          ...
          <h1 style={{ margin: "0 0 6px", fontSize: 28, fontWeight: 800 }}>ProSkill Agency</h1>
          <p style={{ margin: 0, color: colors.textMuted, fontSize: 14 }}>Check your subscription status</p>
        </div>

        {/* Form card */}
        <div style={{ background: colors.cardBg, borderRadius: 14, padding: 20, marginBottom: 20 }}>
          <h2 style={{ margin: "0 0 14px", fontSize: 18, fontWeight: 700 }}>🔍 Find My Subscription</h2>

          <div style={{ marginBottom: 12 }}>
            <label style={{ display: "block", fontSize: 13, color: colors.textMuted, marginBottom: 6, fontWeight: 600 }}>Service</label>
            <select value={selService} onChange={e => setSelService(e.target.value)} style={inputStyle}>
              <option value="all">All my subscriptions</option>
              {services.map(s => <option key={s} value={s}>{s}</option>)}
            </select>
          </div>

          <div style={{ marginBottom: 12 }}>
            <label style={{ display: "block", fontSize: 13, color: colors.textMuted, marginBottom: 6, fontWeight: 600 }}>Your Email *</label>
            <input
              type="email"
              value={email}
          ...
          <div style={{ marginBottom: 14 }}>
            <label style={{ display: "block", fontSize: 13, color: colors.textMuted, marginBottom: 6, fontWeight: 600 }}>Or Phone Number (optional)</label>
            <input
              type="tel"
              value={phone}
              onChange={e => setPhone(e.target.value)}
              placeholder="01270935507"
              style={inputStyle}
            />
          </div>
        ...
        {/* Results */}
        {searched && (
          <div style={{ background: colors.cardBg, borderRadius: 14, padding: 20 }}>
            <h2 style={{ margin: "0 0 14px", fontSize: 18, fontWeight: 700 }}>
              📋 Your Subscriptions ({matchedSubs.length})
            </h2>
            {matchedSubs.length === 0 ? (
              <div style={{ textAlign: "center", padding: 30 }}>
                <div style={{ fontSize: 40, marginBottom: 10 }}>🤔</div>
                <p style={{ color: colors.textMuted, margin: "0 0 14px" }}>
                  We couldn't find any active subscriptions for those details.
                </p>
                <p style={{ color: colors.textMuted, margin: "0 0 20px", fontSize: 13 }}>
                  Make sure your email or phone matches what we have on file.
                </p>
                <a href={"https://wa.me/" + ADMIN_WA + "?text=" + encodeURIComponent("Hi, I checked my status but couldn't find my subscription. Email: " + email)} style={{ ...btnStyle, display: "inline-block", textDecoration: "none" }}>📱 Contact ProSkill</a>
              </div>
            ) : (
              matchedSubs.map(a => {
                const st = subStatus(a);
                const fakeT = { ...colors, dark: true };
                const stColor = statusColor(st, fakeT);
                let statusExtra = "";
                if (st === "active" || st === "expiring_soon") statusExtra = "Renews on " + a.renewDate + " (" + daysLeft(a.renewDate) + " days)";
                else if (st === "expired") statusExtra = "Expired " + Math.abs(daysLeft(a.renewDate)) + " days ago on " + a.renewDate;
                else if (st === "under_maintenance") statusExtra = "Service is currently being upgraded — back soon!";
                else if (st === "lifetime") statusExtra = "Lifetime subscription — never expires!";
                return (
                  <div key={a.id} style={{
                    background: colors.cardBg2, borderRadius: 10,
                    padding: 14, marginBottom: 10,
                    borderLeft: "3px solid " + stColor,
                  }}>
                    <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: 8, gap: 10 }}>
                      <div>
                        <div style={{ fontSize: 16, fontWeight: 700 }}>{a.service}</div>
                        <div style={{ fontSize: 12, color: colors.textMuted, marginTop: 2 }}>Activated since {a.soldDate}</div>
                      </div>
                      <span style={{
                    ...
                    <div style={{ fontSize: 13, color: colors.textMuted, marginTop: 8 }}>{statusExtra}</div>
                    {st === "expiring_soon" || st === "expired" ? (
                      <a
                        href={"https://wa.me/" + ADMIN_WA + "?text=" + encodeURIComponent("Hi, I'd like to renew my " + a.service + " subscription")}
                        style={{
        ...
        {/* Footer */}
        <div style={{ textAlign: "center", marginTop: 30, fontSize: 12, color: colors.textMuted }}>
          <p>Need help? <a href={"https://wa.me/" + ADMIN_WA} style={{ color: colors.primary, textDecoration: "none" }}>Contact us on WhatsApp</a></p>
          <p style={{ marginTop: 4 }}>© ProSkill Digital Agency</p>
        </div>
      </div>
    </div>
  );
...
export default function App() {
  // ═══ CUSTOMER SELF-CHECK MODE — completely separate from main app ═══
  // Detected via URL param ?check=1
  const isCheckMode = (() => {
    if (typeof window === "undefined") return false;
    const p = new URLSearchParams(window.location.search);
    return p.get("check") === "1";
  })();
  if (isCheckMode) {
    return <CustomerCheckPage />;
  }
      ...
      <div style={{
        ...
        {tab === "reports" && (
          <div>
            ...
            {["sales", "profit", "payment", "team", "customers", "inventory", "insights", "bi", "refunds", "statuses"].includes(repTab) && (
            ...
            {repTab === "customers" && (() => {
              ...
              return (
                <div>
                  ...
                  <div style={t.card}>
                    <h3 style={{ margin: "0 0 12px", fontSize: t.fs.md, fontWeight: 700 }}>🏆 Top 20 Customers (by lifetime value)</h3>
                    {custs.length === 0 ? (
            ...
            {repTab === "statuses" && (() => {
              ...
              return (
                <div>
                  ...
                  {/* Customer-facing page link */}
                  <div style={{ ...t.card, marginTop: 14, borderLeft: "3px solid " + t.primary }}>
                    <h3 style={{ margin: "0 0 8px", fontSize: t.fs.md, fontWeight: 700 }}>🌐 Customer Self-Check Page</h3>
                    <p style={{ margin: "0 0 10px", fontSize: t.fs.sm, color: t.textMuted, lineHeight: 1.6 }}>
                      Send this URL to customers via WhatsApp. They pick a service and enter their email to view their subscription status — no login needed.
                    </p>
                    <div style={{
                      background: t.cardBg2, padding: 10, borderRadius: 8,
                      fontFamily: "monospace", fontSize: t.fs.sm,
                      wordBreak: "break-all", marginBottom: 8,
                    }}>
                      {window.location.origin}/?check=1
                    </div>
                    <button
                      onClick={() => {
                        const url = window.location.origin + "/?check=1";
                        copyToClipboard(url);
                        alert("Customer page URL copied!");
                      }}
                      style={t.btnPrimary}
                    >📋 Copy URL</button>
                  </div>
