...
export default function App() {
  ...
  const addSaleEntry = () => {
    ...
    // ═══ DUPLICATE DETECTION: same customer + same service with active subscription ═══
    if (!newSale._dupConfirmed) {
      ...
      lines.forEach(L => {
        const existing = sales.find(a =>
          ...
          && a.service === L.service
          && a.done
          && daysLeft(a.renewDate) > 0
        );
        if (existing) {
          dupes.push({
            service: L.service,
            existing,
            daysLeft: daysLeft(existing.renewDate),
          });
        }
      });
      if (dupes.length > 0) {
        const msg = "⚠️ DUPLICATE SUBSCRIPTION WARNING\n\n" +
          newSale.customer + " already has an active subscription for:\n\n" +
          dupes.map(d =>
            "• " + d.service + " — expires in " + d.daysLeft + " days (on " + d.existing.renewDate + ")"
          ).join("\n") +
          "\n\nDo you want to add another sale anyway?\n(Yes = save / Cancel = stop)";
        if (!confirm(msg)) {
          return;
        }
        // Mark as confirmed so user doesn't see prompt again on retry
