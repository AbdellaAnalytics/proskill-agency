  ...
  async saveData(workspaceOwnerId, data, adminEmail) {
    ...
    for (let attempt = 1; attempt <= 3; attempt++) {
      try {
        ...
        if (r.ok) {
          // ALSO update/create the email-keyed index entry so members can find this workspace
          if (adminEmail) {
            const emailKey = "ws_email_" + adminEmail.toLowerCase();
            const indexBody = JSON.stringify({ data, updated_at: new Date().toISOString() });
            const ri = await this.req("/rest/v1/proskill_workspace?workspace_key=eq." + encodeURIComponent(emailKey), { method: "PATCH", prefer: "return=minimal", body: indexBody });
            if (!ri.ok) {
              // Fallback: insert
              await this.req("/rest/v1/proskill_workspace", { method: "POST", prefer: "return=minimal", body: JSON.stringify({ owner_id: workspaceOwnerId, workspace_key: emailKey, data }) }).catch(() => {});
            }
          }
          return r;
        }
        // Fallback: INSERT new row (only on first attempt)
        if (attempt === 1) {
