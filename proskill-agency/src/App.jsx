...
const sb = {
  ...
  async saveData(workspaceOwnerId, data, adminEmail) {
    ...
    const key = "ws_" + workspaceOwnerId;
    // Use UPSERT (POST with Prefer: resolution=merge-duplicates) so first-time saves work correctly
    // This is more reliable than PATCH-then-POST fallback because PATCH returns OK even when 0 rows match
    const mainBody = JSON.stringify({ owner_id: workspaceOwnerId, workspace_key: key, data, updated_at: new Date().toISOString() });
    for (let attempt = 1; attempt <= 3; attempt++) {
      try {
        // UPSERT the main workspace row
        const r = await this.req("/rest/v1/proskill_workspace", {
          method: "POST",
          prefer: "return=minimal,resolution=merge-duplicates",
          body: mainBody,
        });
        if (r.ok) {
          // ALSO upsert the email-keyed index entry so members can find this workspace
          if (adminEmail) {
            const emailKey = "ws_email_" + adminEmail.toLowerCase();
            const indexBody = JSON.stringify({ owner_id: workspaceOwnerId, workspace_key: emailKey, data, updated_at: new Date().toISOString() });
            try {
              await this.req("/rest/v1/proskill_workspace", {
                method: "POST",
                prefer: "return=minimal,resolution=merge-duplicates",
                body: indexBody,
              });
            } catch (e) { /* ignore, main save already succeeded */ }
          }
          return r;
        }
      } catch (e) { /* retry */ }
...
export default function App() {
  ...
  useEffect(() => {
    ...
    // 3. Debounce: wait 2s after last change before hitting cloud
    clearTimeout(saveTimer);
    saveTimer = setTimeout(async () => {
      // If another save is in flight, pendingData will be picked up after it finishes
      ...
      const doSave = async () => {
        ...
        try {
          ...
          if (r.ok) {
            // Only set "saved" if no new pending data
            if (!pendingData) setSyncStatus("saved");
          } else {
            setSyncStatus("error");
            // Re-queue the failed snapshot if nothing newer queued
  ...
  const approveProof = (saleId) => {
    if (!isAdmin && !can("approveProofs")) { alert("You don't have permission to approve proofs."); return; }
  ...
  const rejectProof = (saleId) => {
    if (!isAdmin && !can("approveProofs")) { alert("You don't have permission to reject proofs."); return; }
    if (!confirm("Reject this proof?")) return;
    setSales(p => p.map(s => s.id === saleId ? { ...s, proofStatus: "rejected" } : s));
  };
  const removeProof = (saleId) => {
    if (!isAdmin && !can("approveProofs")) { alert("You don't have permission to remove proofs."); return; }
