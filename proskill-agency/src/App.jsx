  ...
  const onFile = async (file) => {
    ...
    try {
      ...
      const out = rowsArr.slice(1).map(r => ({
        ...
        endDate: toDateStr(r[4]),
        soldTo: String(r[8] || "").trim(),
        orderNumber: String(r[9] || "").trim(),
        customerPlan: String(r[13] || "").trim(),
...
export default function App() {
  ...
  const importAdobeFromExcel = (parsedRows, mode = "smart") => {
    if (!isAdmin) { alert("Admin only"); return; }
    let addedAccs = 0, addedRentals = 0, skipped = 0;
    ...
    parsedRows.forEach(row => {
      ...
      if (!acc) {
        ...
        // Overwrite credentials too
        Object.assign(acc, {
          passEmail: row.passEmail || acc.passEmail,
          passAdobe: row.passAdobe || acc.passAdobe,
        });
      }
      // Create a rental if dates + customer exist
      if (row.startDate && row.endDate && (row.soldTo || row.customerPlan)) {
        // Check if a rental already exists for this email + dates
        const dup = newRentals.find(r => r.accountId === acc.id
        ...
        if (!dup) {
          newRentals.push({
            ...
            accountId: acc.id,
            saleId: null, // unlinked, since old data
            customer: row.soldTo || "",
            phone: "",
            startDate: row.startDate,
            endDate: row.endDate,
            planMonths: parsePlanMonths(row.customerPlan),
            orderNumber: row.orderNumber || "",
            status: isAfterDate(row.endDate, todayStr()) ? "active" : "completed",
    ...
    addLog("📤 Imported Adobe: " + addedAccs + " accounts, " + addedRentals + " rentals, " + skipped + " skipped");
    return { addedAccs, addedRentals, skipped };
  };
  ...
  }, [sales, scopedSales, svcNames, stockStatsByProduct, adobePendingReminders, adobeOverdue, isAdmin, dismissedN, adobeRentals]);
            ...
            <div>
              ...
              <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 14, flexWrap: "wrap", gap: 10 }}>
                ...
                <div style={{ display: "flex", gap: 8, flexWrap: "wrap" }}>
                  ...
                  {canEdit && (
                    <button onClick={() => setNewAdobeAccount({ email: "", passEmail: "", passAdobe: "", source: "", note: "", banned: false })} style={t.btnPrimary}>+ New Account</button>
                  )}
                </div>
              </div>
              ...
              <div style={{ display: "grid", gridTemplateColumns: isMobile ? "1fr 1fr" : "repeat(5, 1fr)", gap: t.sp.md, marginBottom: 14 }}>
                ...
                <div style={{ ...t.card, textAlign: "center", padding: 14, borderTop: "3px solid #f97316" }}>
                  <div style={{ fontSize: t.fs.xs, color: t.textMuted, fontWeight: 700 }}>🟠 EXPIRING ≤7d</div>
                      ...
                      return (
                        <div key={acc.id} style={{ ...t.card, marginBottom: 10, padding: 14, borderLeft: "3px solid " + stColor }}>
                          <div style={{ display: "flex", justifyContent: "space-between", gap: 8, marginBottom: 6 }}>
                            ...
                            <span style={{
                              fontSize: t.fs.xs, fontWeight: 700, color: stColor, whiteSpace: "nowrap"
                            }}>{acc.banned ? "🚫 Banned" : active ? "🟢 Rented" : "📦 Free"}</span>
                          </div>
                          {active && r && (
                            <div style={{ fontSize: t.fs.xs, color: t.textMuted, marginBottom: 4 }}>
                              👤 {r.customer} · ends {r.endDate} ({r.daysToEnd}d left)
                            </div>
                          )}
                        ...
                        <table style={{ width: "100%", borderCollapse: "collapse", minWidth: 900, fontSize: t.fs.sm }}>
                          <thead>
                            <tr style={{ background: t.cardBg2 }}>
                              {["Email", "Status", "Current Rental", "Ends", "Source", "Note", "Actions"].map(h => (
                                <th key={h} style={{ padding: "10px 12px", textAlign: "left", fontSize: t.fs.xs, color: t.textMuted, fontWeight: 700, textTransform: "uppercase" }}>{h}</th>
                              ))}
                            </tr>
                          </thead>
                            ...
                            {filteredAccounts.map(acc => {
                              ...
                              return (
                                <tr key={acc.id} style={{ borderBottom: "1px solid " + t.border }}>
                                  ...
                                  <td style={{ padding: "10px 12px", fontSize: t.fs.xs }}>
                                    {active && r ? r.endDate + " (" + daysLeft(r.endDate) + "d)" : "—"}
                                  </td>
                                  <td style={{ padding: "10px 12px", fontSize: t.fs.xs, color: t.textMuted }}>{acc.source || "—"}</td>
                                  <td style={{ padding: "10px 12px", fontSize: t.fs.xs, color: t.textMuted, maxWidth: 200, overflow: "hidden", textOverflow: "ellipsis" }}>{acc.note || "—"}</td>
                                  <td style={{ padding: "10px 12px" }}>
                                    {canEdit && (
                                      <div style={{ display: "flex", gap: 4 }}>
