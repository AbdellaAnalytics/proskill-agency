...
const sb = {
  ...
  async saveData(workspaceOwnerId, data) {
    ...
    const r2 = await this.req("/rest/v1/proskill_workspace", { method: "POST", prefer: "return=minimal", body: JSON.stringify({ owner_id: workspaceOwnerId, workspace_key: key, data }) });
    return r2;
  },
};
...
const MEMBER_TABS = [
  { id: "dashboard",   label: "📊 Dashboard (own stats)",     defaultOn: true  },
  { id: "sales",       label: "🔑 Sales (own only)",          defaultOn: true  },
  { id: "customers",   label: "👤 Customers (own only)",      defaultOn: true  },
  { id: "tasks",       label: "✅ Tasks (assigned)",           defaultOn: true  },
  { id: "commission",  label: "💰 Commission (own earnings)",  defaultOn: true  },
  { id: "stock",       label: "📦 Stock (full access)",        defaultOn: true  },
  { id: "adobe",       label: "🎨 Adobe (full access)",        defaultOn: true  },
  { id: "guide",       label: "📋 Guide (read-only)",          defaultOn: true  },
];
...
export default function App() {
  ...
  const loadWorkspace = async (user) => {
    ...
    try {
      ...
      if ((user.email || "").toLowerCase() === ADMIN_EMAIL.toLowerCase()) {
        ...
        const d = await sb.loadData(user.id);
        applyLoadedData(d);
        setLoaded(true);
        return;
      }
      // Member: use admin's id from localStorage (set when admin last logged in here)
      const adminId = localStorage.getItem("ps_admin_id");
      if (adminId) {
        setWorkspaceOwnerId(adminId);
        const d = await sb.loadData(adminId);
        applyLoadedData(d);
        setLoaded(true);
        return;
      }
  ...
  useEffect(() => {
    ...
    saveTimer = setTimeout(async () => {
      const r = await sb.saveData(workspaceOwnerId, {
        ...
        <div style={{
          ...
          <button
            ...
            style={{
              ...
              border: "2px solid #2a9d8f", borderRadius: 10,
          ...
          <p style={{ margin: "20px 0 0", fontSize: 11, color: "#94a3b8", lineHeight: 1.5 }}>
            ...
            Admin: use your main account to manage everything.
          </p>
        </div>
      </div>
  );
  ...
  const addSaleEntry = () => {
    if (!newSale) return;
    const err = validateSale(newSale);
    if (err) { alert(err); return; }
    const rD = newSale.period > 0
      ? addMonths(newSale.soldDate || todayStr(), newSale.period)
      : newSale.period === -1 ? "2099-12-31" : newSale.soldDate || todayStr();
    ...
    // If member is creating, auto-assign to themselves
    ...
    ensureCustomer(entry.customer, entry.customerPhone, entry.customerEmail);
    setSales(p => [entry, ...p]);
    if (available) {
  ...
  const canEditSale = (s) => isAdmin || (s.assignedTo === memberId);
  ...
  const approveProof = (saleId) => {
    if (!isAdmin) { alert("Only admins can approve proofs."); return; }
  ...
  const canEditTask = (tk) => isAdmin || tk.assignedTo === memberId;
  ...
  const scopedSales = useMemo(() => {
    return isAdmin ? sales : sales.filter(a => a.assignedTo === memberId);
  ...
  const mrr = useMemo(() => {
    const act = scopedSales.filter(a => a.done && daysLeft(a.renewDate) > 0);
    const mo = act.reduce((s, a) => s + ((a.priceEGP || a.price || 0) / (a.period || 1)), 0);
    const ch = scopedSales.filter(a => a.done && daysLeft(a.renewDate) < 0).length;
  ...
  const alerts = useMemo(() => {
    ...
    const pendingProofs = baseSales.filter(a => a.proofStatus === "pending");
  ...
  const custList = useMemo(() => {
    const visibleCustNames = isAdmin ? null : new Set(scopedSales.map(a => (a.customer || "").toLowerCase()));
    const source = isAdmin ? customers : customers.filter(c => visibleCustNames.has(c.name.toLowerCase()));
    return source.map(c => {
      const cs = scopedSales.filter(a => a.customer && a.customer.toLowerCase() === c.name.toLowerCase());
      const d = cs.filter(a => a.done);
  ...
  }, [customers, scopedSales, custHealth, isAdmin]);
  ...
  const filteredSales = useMemo(() => {
    ...
    return sl;
  }, [scopedSales, search, dFrom, dTo, salesFilter, salesFilterProd, salesFilterPhone]);
  ...
  const visibleTasks = useMemo(() => {
    return isAdmin ? tasks : tasks.filter(tk => tk.assignedTo === memberId);
  ...
  const commissionReport = useMemo(() => {
    ...
    const viewTeam = isAdmin ? team : team.filter(m => m.id === memberId);
  ...
  const syncLabel = syncStatus === "saved" ? "☁️ Synced" : syncStatus === "saving" ? "Saving..." : "Sync error";
        ...
        <div style={{
          ...
          <div style={{ display: "flex", alignItems: "center", gap: 10, minWidth: 0, flex: 1 }}>
            ...
            <div style={{ minWidth: 0 }}>
              ...
              <div style={{ color: syncColor, fontSize: 10, display: "flex", alignItems: "center", gap: 4 }}>
      ...
      <div style={{
          ...
          >
            ...
            >
              <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 14 }}>
                <h3 style={{ margin: 0, fontSize: t.fs.lg, fontWeight: 700 }}>🔔 Notifications</h3>
        ...
        {showFU && (
          ...
          >
            ...
            >
              <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 14 }}>
                <h3 style={{ margin: 0, fontSize: t.fs.lg, fontWeight: 700 }}>📞 Follow Up ({alerts.fu.length})</h3>
        ...
        {selSale && (
          ...
          >
            ...
            >
              <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 14 }}>
                <h3 style={{ margin: 0, fontSize: t.fs.lg, fontWeight: 700 }}>{svcIcon(selSale.service)} {selSale.service}</h3>
        ...
        {editSale && (
          ...
          >
            ...
            >
              <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 14 }}>
                <h3 style={{ margin: 0, fontSize: t.fs.lg, fontWeight: 700 }}>✎ Edit Sale</h3>
        ...
        {proofModal && (() => {
          ...
          return (
            ...
            >
              ...
              >
                <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 14 }}>
                  <h3 style={{ margin: 0, fontSize: t.fs.lg, fontWeight: 700 }}>📎 Payment Proof</h3>
                ...
                {!sale.paymentProof ? (
                  <div>
                    <p style={{ fontSize: t.fs.base, color: t.textMuted, marginBottom: 14 }}>
                      Upload payment proof (JPG, PNG, or PDF — max 5MB)
                    </p>
                    <input
                      type="file"
                  ...
                  <div>
                    <div style={{ background: t.cardBg2, borderRadius: 10, padding: 12, marginBottom: 14, fontSize: t.fs.sm, lineHeight: 1.6 }}>
                      <div><strong>File:</strong> {sale.paymentProof.name}</div>
                      <div><strong>Size:</strong> {Math.round((sale.paymentProof.size || 0) / 1024)} KB</div>
                      <div><strong>Uploaded by:</strong> {sale.paymentProof.uploadedBy}</div>
                      <div><strong>Status:</strong> <span style={{ color: sale.proofStatus === "approved" ? t.success : sale.proofStatus === "rejected" ? t.danger : t.warning, fontWeight: 700 }}>{(sale.proofStatus || "pending").toUpperCase()}</span></div>
                    </div>
                    {sale.paymentProof.type && sale.paymentProof.type.startsWith("image/") && (
                      <img src={sale.paymentProof.data} alt="Proof" style={{ maxWidth: "100%", maxHeight: 400, borderRadius: 8, marginBottom: 14, display: "block" }} />
                    )}
                    <div style={{ display: "flex", gap: 8, flexWrap: "wrap" }}>
                      <a href={sale.paymentProof.data} download={sale.paymentProof.name} style={{ ...t.btnGhost, textDecoration: "none", textAlign: "center", display: "inline-flex", alignItems: "center", justifyContent: "center", flex: 1, minWidth: 120 }}>⬇ Download</a>
                      {isAdmin && sale.proofStatus !== "approved" && (
                        <button onClick={() => approveProof(sale.id)} style={{ ...t.btnPrimary, background: t.success, flex: 1, minWidth: 120 }}>✓ Approve</button>
                      )}
                      {isAdmin && sale.proofStatus !== "rejected" && (
                        <button onClick={() => rejectProof(sale.id)} style={{ ...t.btnDanger, flex: 1, minWidth: 120 }}>✕ Reject</button>
                      )}
                      {isAdmin && (
                        <button onClick={() => removeProof(sale.id)} style={{ ...t.btnGhost, color: t.danger, borderColor: t.danger, flex: 1, minWidth: 120 }}>🗑 Remove</button>
                      )}
                    </div>
                  </div>
                )}
          ...
          >
            ...
            >
              <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 16 }}>
                <h3 style={{ margin: 0, fontSize: t.fs.lg, fontWeight: 700 }}>💾 Backup &amp; Restore</h3>
              ...
              <div style={{ padding: isMobile ? "18px 20px" : "22px 26px" }}>
                <div style={{ display: "grid", gridTemplateColumns: isMobile ? "1fr" : "1fr 1fr", gap: 14, marginBottom: 18 }}>
                  <div>
                    ...
                    {invSale.customerPhone && <p style={{ margin: 0, fontSize: 11, color: "#64748b" }}>📞 {invSale.customerPhone}</p>}
                  </div>
        ...
        {selCust && (
          ...
          >
            ...
            >
              <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 14 }}>
                <h3 style={{ margin: 0, fontSize: t.fs.lg, fontWeight: 700 }}>👤 {selCust.name}</h3>
        ...
        {tab === "dashboard" && (
          <div>
            ...
            <div style={{
              ...
              {[
                { l: "Today", d: myTime.daily, cl: t.warning },
                { l: "Week",  d: myTime.weekly, cl: t.primary },
                { l: "Month", d: myTime.monthly, cl: t.primaryDark },
                { l: "Year",  d: myTime.yearly, cl: t.success },
              ].map((p, i) => (
                <div key={i} style={{ ...t.cardCompact, borderTop: "2px solid " + p.cl }}>
                  ...
                  <p style={{ margin: "2px 0 0", fontSize: t.fs.lg, fontWeight: 900, color: p.cl, lineHeight: 1 }}>{p.d.count}</p>
                  <p style={{ margin: 0, fontSize: t.fs.xs, color: t.textMuted }}>{p.d.rev.toLocaleString()}</p>
                </div>
              ))}
            </div>
            ...
            {newSale && (
              <div style={{ ...t.card, marginBottom: 14, border: "2px solid " + t.primary }}>
                ...
                <div style={{ display: "grid", gridTemplateColumns: isMobile ? "1fr" : "1fr 1fr", gap: 10, marginBottom: 10 }}>
                  <div>
                    <label style={t.label}>Service</label>
                    <select value={newSale.service} onChange={e => setNewSale(p => ({ ...p, service: e.target.value }))} style={t.input}>
                      {svcNames.map(s => <option key={s}>{s}</option>)}
                    </select>
                  </div>
                  <div>
                    <label style={t.label}>Customer *</label>
                    <input value={newSale.customer} onChange={e => setNewSale(p => ({ ...p, customer: e.target.value }))} style={{ ...t.input, borderColor: !newSale.customer.trim() ? t.danger : undefined }} />
                  </div>
                </div>
                <div style={{ display: "grid", gridTemplateColumns: isMobile ? "1fr" : "1fr 1fr", gap: 10, marginBottom: 10 }}>
                  <div>
                ...
                <div style={{ display: "grid", gridTemplateColumns: isMobile ? "1fr 1fr" : "repeat(5, 1fr)", gap: 10, marginBottom: 10 }}>
                  <div>
                    <label style={t.label}>Period</label>
                    <select value={newSale.period} onChange={e => setNewSale(p => ({ ...p, period: +e.target.value }))} style={t.input}>
                      {PERIODS.map(p => <option key={p} value={p}>{PERIOD_LABEL(p)}</option>)}
                    </select>
                  </div>
                  <div>
                    <label style={t.label}>Price *</label>
                    <input type="number" value={newSale.price} onChange={e => setNewSale(p => ({ ...p, price: +e.target.value }))} style={{ ...t.input, borderColor: newSale.price <= 0 ? t.danger : undefined }} />
                  </div>
                  <div>
                    <label style={t.label}>Cost</label>
                    <input type="number" value={newSale.costPrice} onChange={e => setNewSale(p => ({ ...p, costPrice: +e.target.value }))} style={t.input} />
                  </div>
                  <div>
                    <label style={t.label}>Currency</label>
                    <select value={newSale.currency} onChange={e => setNewSale(p => ({ ...p, currency: e.target.value }))} style={t.input}>
                      {CURRENCIES.map(c => <option key={c.code} value={c.code}>{c.code}</option>)}
                    </select>
                  </div>
                  <div>
                ...
                {isAdmin && team.length > 0 && (
                  <div style={{ marginBottom: 10 }}>
                    <label style={t.label}>👤 Sold By (Team Member)</label>
                ...
                <div style={{ display: "flex", gap: 8 }}>
                  ...
                  <button onClick={addSaleEntry} style={{ ...t.btnPrimary, flex: 1 }}>💾 Add Sale</button>
                </div>
              </div>
            )}
                    ...
                    >
                      ...
                      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", gap: 8, fontSize: t.fs.sm, color: t.textMuted, flexWrap: "wrap" }}>
                        ...
                        {a.assignedTo && (() => {
                          ...
                          return m ? <span style={{ color: t.primary, fontWeight: 600 }}>👤 {m.name}</span> : null;
                        })()}
                      </div>
          ...
          <div>
            ...
            {editTask && (
              ...
              >
                ...
                >
                  <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 14 }}>
                    <h3 style={{ margin: 0, fontSize: t.fs.lg, fontWeight: 700 }}>✎ Edit Task</h3>
          ...
          <div>
            ...
            {newMember && (
              <div style={{ ...t.card, marginBottom: 14, border: "2px solid " + t.primary }}>
                ...
                <div style={{
                  ...
                  <div style={{ fontSize: t.fs.sm, fontWeight: 700, marginBottom: 10 }}>
                    🔒 TAB PERMISSIONS (what this member will see when they log in)
                  </div>
                  <div style={{
                    display: "grid",
                    gridTemplateColumns: isMobile ? "1fr" : "1fr 1fr",
            ...
            {editMember && (
              ...
              >
                ...
                >
                  <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 14 }}>
                    <h3 style={{ margin: 0, fontSize: t.fs.lg, fontWeight: 700 }}>✏️ Edit {editMember.name}</h3>
                  ...
                  <div style={{ background: t.dark ? "#0f172a" : "#f5f7f9", borderRadius: 10, padding: 14, marginBottom: 12 }}>
                    <div style={{ fontSize: t.fs.sm, fontWeight: 700, marginBottom: 10 }}>🔒 Tab Permissions</div>
          ...
          <div>
            ...
            {editGuide && isAdmin && (
              ...
              >
                ...
                >
                  <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 14 }}>
                    <h3 style={{ margin: 0, fontSize: t.fs.lg, fontWeight: 700 }}>✎ Edit Guide</h3>
